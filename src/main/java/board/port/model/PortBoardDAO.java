 ...
 * DataSource dataSource;
 * 
 * public PortBoardDAO() { try { Context context=new InitialContext(); Context
 * envContext= (Context)context.lookup("java:/comp/env");
 * dataSource=(DataSource)envContext.lookup("jdbc/oracle");
 * System.out.println("오라클 DBMS의 커넥션풀이 완성되었습니다. 언제든지 Connection을 꺼내 사용"); //커넥션
 * 디비랑 연결하는 애 } catch (NamingException e) { // TODO Auto-generated catch block
 * e.printStackTrace(); }
 * 
 * }//생성자 //전체조회// public List<PortBoardVO> selectAllBoard(){ List<PortBoardVO>
 * portBoardList =new ArrayList<PortBoardVO>();
 * 
 * 
 * 
 * try { Connection conn = dataSource.getConnection(); String sql=
 * "SELECT * FROM PORT_BOARD"; PreparedStatement
 * pstmt=conn.prepareStatement(sql);
 * 
 * ResultSet rs=pstmt.executeQuery();
 * 
 * while(rs.next()) { PortBoardVO portBoard= new PortBoardVO();
 ...
 * 
 * portBoardList.add(portBoard); } rs.close(); pstmt.close(); conn.close();
 * 
 ...
 * 
 * //이미지 경로 조회 //수정해야 할 수도 있음. public PortBoardVO selectImagePath(PortBoardVO
 * vo) {
 * 
