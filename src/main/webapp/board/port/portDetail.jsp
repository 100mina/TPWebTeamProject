...
>>>>>>> refs/heads/민아
	<script type="text/javascript">
		...
		function isLike() {
        	var xhr = new XMLHttpRequest();
        	xhr.open("GET", "portIsLike?user_id=${port.userId}&port_no=${port.portNo}&is_like=${port.isLike}", true);
        	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        	xhr.onreadystatechange = function () {
            	if (xhr.readyState == 4 && xhr.status == 200) {
            		location.reload();
...
<body>
	...
	<div id="bottom">
		<p id="bottom_title">
			<strong>모든 포트폴리오</strong>
		</p>
		<div id="port_all_container">
			<c:forEach var="port" items="${port.userPort }">
				<div style="text-align: center; margin: 20px 10px;">
					<a href="/TPWebTeamProject/getPortDetail?port_no=${port.portNo} "><img
						src="/TPWebTeamProject/getImg?img_no=${port.portImg.imgNo}" alt=""
						class="port_all"></a><br> ${port.portTitle }
				</div>
			</c:forEach>
		</div>
	</div>
	...
	<div id="comment_container">
		...
		<c:forEach var="cmt" items="${port.portCmtList }">
			...
			<div id="comment_item">
				<a href=""><img alt="img"
					src="/TPWebTeamProject/board/port/img/profile.png"
					id="comment_profile"></a>
				<h5>${cmt.userId }</h5>
				...
				<div id="cmt_button">
					<a
						href="javascript:updateCmt('${cmt.portCmtNo }','${cmt.userId }','${formattedCmtContent}',${cmt.portNo })">수정</a>&nbsp;&nbsp;&nbsp;
					<a class="deleteLink"
						href="/TPWebTeamProject/deletePortCmt?port_cmt_no=${cmt.portCmtNo }&port_no=${cmt.portNo }">삭제</a>
				</div>
		...
		<div id="input_container">
			<form action="/TPWebTeamProject/insertPortCmt" method="post"
				enctype="application/x-www-form-urlencoded"
				style="margin: 20px 40px;">
				<textarea rows="5" placeholder="댓글 남기기" maxlength="1000"
					name="port_cmt_content"></textarea>
				<br> <input type="hidden" name="port_no" value="${port.portNo}">
				<input type="submit" value="완료" class="input_button"
					id="scrollTarget">&nbsp;&nbsp;&nbsp; <input type="reset"
					value="취소" class="input_button">
			</form>
		</div>
